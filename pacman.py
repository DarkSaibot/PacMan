...
def draw_misc():
    ...
    if game_over:
        ...
        gameover_text = font.render('Fim De Jogo! Pressione a barra de espaço para reiniciar!', True, 'red')
        screen.blit(gameover_text, (100, 300))
    if game_won:
        pygame.draw.rect(screen, 'white', [50, 200, 800, 300],0, 10)
        pygame.draw.rect(screen, 'dark gray', [70, 220, 760, 260], 0, 10)
        gameover_text = font.render('VITÓRIA! Pressione a barra de espaço para reiniciar!', True, 'green')
        screen.blit(gameover_text, (100, 300))
...
while run:
    ...
    if not powerup:
        if (player_circle.colliderect(blinky.rect) and not blinky.dead) or (player_circle.colliderect(inky.rect) and not inky.dead) or \
        (player_circle.colliderect(pinky.rect) and not pinky.dead) or (player_circle.colliderect(clyde.rect) and not clyde.dead):
            if lives > 0:
                lives -= 1
                startup_counter = 0
                powerup = False
